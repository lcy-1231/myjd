(function(){
  ...
  Tab.prototype.init = function (options){
    ...
    this.setClass(this.cons[this.showIndex],'tab-head-item show')
    // 绑定事件
    this.bindEvent()
  }
  // 绑定事件方法
  ...
  init1.style.className='top-tab-lk active2'
    ...
    for (var i = 0, len = this.tits.length; i < len; i++){
      ...
      this.tits[i].onmouseover = function (){
        ...
        _this.setClass(_this.cons[_this.showIndex],'tab-body-item')
        // 当前选中元素添加类名
        _this.setClass(_this.tits[this.index],'top-tab-lk active2')
        _this.setClass(_this.cons[this.index],'tab-body-item show2')
        // 更新当前显示的下标
