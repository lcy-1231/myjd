...
function jsonp(options) {
  ...
  window[options.jsonpCallback] = options.success; //判断options.data的数据类型
  //如果是字符串，直接赋值data变量
  //如果是对象，转成参数序列的字符串
  ...
  var oScript = document.createElement('script'); //给 src属性赋值(url+接口参数)

  oScript.src = options.url + '?' + options.jsonp + '=' + options.jsonpCallback + '&' + data; //把script插入到文档中
