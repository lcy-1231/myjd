...
ajax({
  ...
  success: function success(json) {
    ...
    json.forEach(function (item) {
      //取到id为id的一条数据
      if (id === item.id) {
        html += "\n          <div class=\"main\">\n            <div class=\"left\">\n              <div class=\"minBox\">\n                <img src=\"".concat(item.xuan1, "\" alt=\"\">\n                <div class=\"mask\"></div>\n              </div>\n              <div class=\"opt\">\n                <div id=\"box\" class=\"xuan\">\n                  <img src=\"").concat(item.xuan1, "\" alt=\"\">\n                </div>\n                <div id=\"box\">\n                  <img src=\"").concat(item.xuan2, "\" alt=\"\">\n                </div>\n                <div id=\"box\">\n                  <img src=\"").concat(item.xuan3, "\" alt=\"\">\n                </div>\n              </div>\n            </div>\n            <div class=\"content\">\n              <img src=\"").concat(item.con1, "\" alt=\"\">\n            </div>\n            <div class=\"maxBox\">\n              <img src=\"").concat(item.xuan1, "\" alt=\"\">\n            </div>\n          </div>\n        ");
        xuanran.innerHTML += html; // 获取元素 需要小图 遮罩层 大图 大图中的图片

        var minBox = document.querySelector('.minBox');
        var mask = document.querySelector('.mask');
        var maxBox = document.querySelector('.maxBox');
        var maxImg = document.querySelector('.maxBox img'); // 获取选项卡
        //获取小图里的图片

        var minImg = document.querySelector('.minBox img'); // console.log(minImg);

        var boxsImg = document.querySelectorAll('#box img'); // console.log(boxsImg);
        ...
        for (var i = 0; i < boxsImg.length; i++) {
          ...
          boxsImg[i].onmouseover = function () {
            ...
            boxsImg[this.index].className += ' xuan'; //点击之后改变小图 将小图里的图片变成选项卡里的图片

            minImg.src = boxsImg[this.index].src; //点击之后改变大图
        ...
        minBox.onmousemove = function (eve) {
          ...
          var e = eve || window.event; // 计算mask的定位坐标
          // e.clientX 鼠标的坐标位置
          // offset(minBox).left 小图盒子离浏览器边框的距离
          // 求的是遮罩层在小图里面的坐标位置

          var maskLeft = e.clientX - offset(minBox).left - mask.clientWidth / 2;
          var maskTop = e.pageY - offset(minBox).top - mask.clientHeight / 2; // 求遮罩层在 小图中 的边界范围
          ...
          mask.style.top = maskTop + 'px'; // 计算小图中遮罩层和小图的比例
          // 遮罩层离小图的左边有多远/遮罩层能在小图中移动的距离

          var scaleX = maskLeft / (minBox.clientWidth - mask.clientWidth);
          var scaleY = maskTop / (minBox.clientHeight - mask.clientHeight); // 大图也随着比例运动
